...
5. **Confirm live.** After about a minute, fetch the live URL and check the change actually appears. If it doesn't, check the Cloudflare build (the owner can open their Worker's Deployments in the dashboard; the usual causes are a build error, which you should fix immediately, or DNS still propagating on a fresh setup).
